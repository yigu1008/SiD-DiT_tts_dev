...
# REWARD_SERVER_URL is optional: when set, reward_unified routes via the server;
# when unset, ImageReward loads inline (single-GPU local runs).
if [[ -z "${REWARD_SERVER_URL:-}" ]]; then
    echo "[mcts-param-ablation] REWARD_SERVER_URL not set → loading ImageReward inline (local mode)"
...
# Single GPU unless caller explicitly set CUDA_VISIBLE_DEVICES.
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
# Match NUM_GPUS to the visible-devices count (comma-separated).
if [[ -z "${NUM_GPUS:-}" ]]; then
    NUM_GPUS=$(echo "${CUDA_VISIBLE_DEVICES}" | awk -F, '{print NF}')
fi
export NUM_GPUS
# Mem hygiene + unbuffered logs (helps single-GPU 48GB A100 runs).
...
# Eval can run with ImageReward only — allow missing HPSv3 etc. if user
# narrows EVAL_BACKENDS via env.
...
# Local-mode default: ImageReward only for both search AND eval (skips
# loading HPSv3 — saves ~5GB GPU). Override via env to add hpsv3 etc.
...
echo "[ablation] DONE all (${BACKENDS} × ${CELLS}) cells phase-1."

# ── Phase-2 posthoc eval: HPSv3 + PickScore inline on saved best_images ─────
# Skip with RUN_POSTHOC=0. SD3.5 + ImageReward are unloaded between cells
# (separate torchrun processes), so HPSv3 (~5GB) and PickScore (~3GB) have
# room to load on the same GPU. evaluate_best_images_multi_reward.py loads
# each backend sequentially within one invocation, so we save model-load
# time vs calling it once per backend.
if [[ "${RUN_POSTHOC:-1}" == "1" ]]; then
    echo
    echo "================================================================"
    echo "[ablation] phase-2 posthoc: hpsv3 + pickscore on saved best_images"
    echo "================================================================"
    posthoc_failed=()
    ...
    if (( ${#posthoc_failed[@]} > 0 )); then
        echo "[ablation] phase-2 posthoc WARN failures: ${posthoc_failed[*]}"
    else
        echo "[ablation] phase-2 posthoc DONE."
    fi
fi
