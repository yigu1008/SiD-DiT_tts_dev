#!/usr/bin/env bash
# Idiot-friendly cherry-pick: flux_schnell × ImageReward only.
...
#   BON_MCTS_N_SEEDS=16 (default — 16-seed prescreen)
#   METHODS="baseline fksteering dts_star bon_mcts"  (override to narrow)
...
if curl -s --max-time 3 "${REWARD_SERVER_URL}/health" >/dev/null 2>&1; then
    echo "[flux-ir] reusing reward server at ${REWARD_SERVER_URL}"
else
    echo "[flux-ir] booting reward server on GPU(s) ${CUDA_VISIBLE_DEVICES_REWARD} (port ${REWARD_SERVER_PORT})"
    ...
    for i in $(seq 1 100); do
        ...
        if ! kill -0 "${SERVER_PID}" 2>/dev/null; then
            echo "[flux-ir] FATAL reward server died early; tail of log:" >&2
            tail -n 80 "${SERVER_LOG}" >&2
            exit 1
        fi
        if (( i % 10 == 0 )); then echo "[flux-ir] waiting for server health... (${i}s)"; fi
        sleep 3
    done
    [[ "${HEALTH_OK}" == "1" ]] || { echo "[flux-ir] FATAL server not healthy"; tail -n 80 "${SERVER_LOG}" >&2; exit 1; }
    echo "[flux-ir] reward server READY"
fi

# ── 2. Run flux_schnell × imagereward (auto-shuffle subset via SHUFFLE_ID) ─
echo
echo "================================================================"
echo "[flux-ir] flux_schnell × imagereward"
echo "================================================================"
env \
...
echo "================================================================"
echo "[flux-ir] DONE."
echo "  Output: ${RUN_ROOT}/flux_schnell_imagereward*/_winners/winners.json"
ls -d "${RUN_ROOT}"/flux_schnell_imagereward* 2>/dev/null | tail -5
