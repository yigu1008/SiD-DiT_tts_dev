#!/usr/bin/env bash
# MCTS ablation study for FLUX Schnell: prompt variants / CFG scale
#
# Ablations (Flux does not have reward correction as an action yet):
#   none       – no search actions   (N_VARIANTS=1, CFG=[1.0])
#   prompt     – prompt variants only (N_VARIANTS=3, CFG=[1.0])
#   cfg        – CFG scale only      (N_VARIANTS=1, CFG=all)
#   prompt_cfg – prompt + CFG        (N_VARIANTS=3, CFG=all)
#
# Usage:
#   bash run_mcts_ablation_flux.sh
#   ABLATIONS="none prompt_cfg" bash run_mcts_ablation_flux.sh
#   N_SIMS=20 bash run_mcts_ablation_flux.sh
#
set -euo pipefail
...
OUT_ROOT="${OUT_ROOT:-/mnt/data/v-yigu/mcts_ablation_flux}"
...
ABLATIONS="${ABLATIONS:-none prompt cfg prompt_cfg}"
...
# ---------------------------------------------------------------------------
# Run one ablation config (sharded across GPUs)
# ---------------------------------------------------------------------------
run_ablation() {
  local name="$1"
...
for name in ablation_names:
    ...
    # Collect from per-rank summary.json files (flux layout)
