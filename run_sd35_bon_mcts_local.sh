...
OUT_DIR="${OUT_DIR:-./sd35_bon_mcts_local_out}"
...
if [[ ! -f "${PROMPT_FILE}" ]]; then
  echo "[sd35-bon-mcts] prompt file not found, exporting HPSv2 prompts first ..."
...
if [[ -n "${END_INDEX}" || "${START_INDEX}" != "0" || "${NUM_PROMPTS}" != "0" ]]; then
  if (( NUM_PROMPTS > 0 )); then
    if [[ -n "${END_INDEX}" ]]; then
      echo "[sd35-bon-mcts] both NUM_PROMPTS and END_INDEX are set; using NUM_PROMPTS (END_INDEX ignored)."
    fi
...
with open(dst, "w", encoding="utf-8") as f:
    for p in prompts[start:end]:
        f.write(p + "\n")
print(f"[sd35-bon-mcts] prompt slice: src={src} range=[{start},{end}) n={end-start} -> {dst}")
PY
  PROMPT_FILE_RUN="${SLICE_FILE}"
fi

SD35_BACKEND_VAL="${SD35_BACKEND:-sid}"
...
if (( NUM_GPUS > 1 )); then
  launch_cmd=("${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node "${NUM_GPUS}" "${SCRIPT_DIR}/sd35_ddp_experiment_bon_mcts.py")
else
  launch_cmd=("${PYTHON_BIN}" "${SCRIPT_DIR}/sd35_ddp_experiment_bon_mcts.py")
fi

echo "[sd35-bon-mcts] prompt_file=${PROMPT_FILE_RUN}"
...
echo "[sd35-bon-mcts] refine=${BON_MCTS_REFINE_METHOD:-ours_tree} lookahead_mode=${LOOKAHEAD_METHOD_MODE:-rollout_tree_prior_adaptive_cfg}"
