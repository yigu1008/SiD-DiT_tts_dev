...
where:
  ...
  - action a_t chooses a prompt-conditioning blend
...
ACTION_BANK: list[ActionSpec] = [
    ActionSpec("coarse_only", 1.0, 0.0, 0.0),
    ActionSpec("mid_only", 0.0, 1.0, 0.0),
    ActionSpec("fine_only", 0.0, 0.0, 1.0),
    ActionSpec("coarse_mid_50", 0.5, 0.5, 0.0),
    ActionSpec("mid_fine_50", 0.0, 0.5, 0.5),
]
ACTION_NAME_TO_ID = {a.name: i for i, a in enumerate(ACTION_BANK)}
...
def run_rollout_action_sequence(
    ...
    for step_idx, (t_flat, t_4d) in enumerate(sched):
        ...
        velocity = su.transformer_step(
            args, ctx, latents, pe, pm, cache.ue, cache.um, t_flat, float(args.guidance_scale)
        )
...
def run_rollout_original_prompt(
    ...
    for step_idx, (t_flat, t_4d) in enumerate(sched):
        ...
        latents = (1.0 - t_4d) * dx + t_4d * noise
        velocity = su.transformer_step(
            args, ctx, latents, cache.orig_pe, cache.orig_pm, cache.orig_ue, cache.orig_um, t_flat, float(args.guidance_scale)
        )
        dx = latents - t_4d * velocity
...
def default_fixed_schedule(steps: int) -> list[int]:
    # early -> coarse_mid_50, middle -> mid_only, late -> mid_fine_50/fine_only
    ids: list[int] = []
    for i in range(steps):
        p = float(i) / float(max(1, steps - 1))
        if p < 0.34:
            ids.append(ACTION_NAME_TO_ID["coarse_mid_50"])
        elif p < 0.67:
            ids.append(ACTION_NAME_TO_ID["mid_only"])
        elif p < 0.90:
            ids.append(ACTION_NAME_TO_ID["mid_fine_50"])
        else:
            ids.append(ACTION_NAME_TO_ID["fine_only"])
    return ids
...
def mutate_float(genome: list[float], p: float, rng: np.random.Generator) -> list[float]:
    sigmas = [0.06, 0.06, 0.02, 0.03, 0.15, 0.15, 0.15]
    out = list(genome)
    for i in range(len(out)):
        if rng.random() < p:
